class TempMailExtension {
    ...
    bindEvents() {
        ...
        // • You can open the same inbox on any device with this link
    ...
    async refreshEmails() {
        ...
        try {
            const apiUrl = `${this.apiBase}/fakeemails?email=${this.currentEmail}`;
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch emails. Status:', response.status, 'StatusText:', response.statusText);
                const errorText = await response.text();
                console.error('Error response:', errorText);

                // Show error in UI
                this.showFetchError(`Failed to fetch emails (${response.status})`);
            }
        } catch (error) {
            ...
            // Show error in UI
            this.showFetchError('Network error: Unable to connect to email service');
        } finally {
            this.isRefreshing = false;
    ...
    showFetchError(message) {
        ...
        inboxList.innerHTML = `
            <div class="error-state">
                ...
                <p class="error-suggestion">Please check your internet connection and try again.</p>
    ...
    showEmailModal(email) {
        ...
        if (email.body_html) {
            ...
            body.innerHTML = email.body_text;
        } else {
...
Access link copied to your clipboard!
💡 How to use:
• Use this link to access your inbox anytime within 7 days
• You can open the same inbox on any device with this link
• The link will expire after 7 days for security
...
• Use this link to access your inbox anytime within 7 days
• You can open the same inbox on any device with this link
• The link will expire after 7 days for security`, 'success');
                    }
    ...
    showSaveMessage(message, type = 'info') {
        ...
        if (type === 'success') {
            ...
            // For error/info messages, show as a simple alert or notification
            alert(message);
        }
    }
    ...
    showSuccessModal(message) {
        ...
        // Parse the message and create HTML content
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            } else if (line.startsWith('✅ Email saved successfully for 7 days!') || line.startsWith('✅ Link copied to clipboard!')) {
