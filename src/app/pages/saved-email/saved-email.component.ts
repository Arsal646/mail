    ...
    <div class="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div class="bg-white rounded-lg shadow-lg p-8 max-w-md w-full text-center">
        ...
        <div *ngIf="loading" class="space-y-4">
          ...
          <h2 class="text-xl font-semibold text-gray-900" i18n="@@savedEmail.verifying.title">Verifying Access Link...</h2>
          <p class="text-gray-600" i18n="@@savedEmail.verifying.subtitle">Please wait while we verify your saved email.</p>
        </div>

        <!-- Success State -->
        <div *ngIf="!loading && emailData" class="space-y-4">
          <lucide-icon name="check-circle" [size]="48" class="text-green-600 mx-auto"></lucide-icon>
          <h2 class="text-xl font-semibold text-gray-900" i18n="@@savedEmail.success.title">Access Verified!</h2>
          <p class="text-gray-600" i18n="@@savedEmail.success.subtitle">Redirecting to your saved email inbox...</p>
          <div class="bg-gray-50 p-3 rounded-lg">
            <p class="text-sm font-mono text-gray-800">{{emailData.email_address}}</p>
            <p class="text-xs text-gray-500 mt-1" i18n="@@savedEmail.daysRemaining">{{emailData.days_remaining}} days remaining</p>
          </div>
        </div>
        ...
        <div *ngIf="!loading && error" class="space-y-4">
          ...
          <h2 class="text-xl font-semibold text-gray-900" i18n="@@savedEmail.error.title">Access Link Invalid</h2>
          <p class="text-gray-600">{{error}}</p>
          <button (click)="goHome()" 
                  class="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors" i18n="@@savedEmail.error.generateNew">
            Generate New Email
          </button>
        </div>
